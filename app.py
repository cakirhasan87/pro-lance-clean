...
# Chatbot AI Responses - Türkçe ve İngilizce
CHATBOT_RESPONSES = {
    'tr': {
        'greeting': [
            'Merhaba! Ben Çetin, Pro-Lance destek asistanıyım. Size nasıl yardımcı olabilirim?',
            'Selam! Pro-Lance hizmetleri hakkında sorularınızı yanıtlamak için buradayım.',
            'Hoş geldiniz! Size nasıl yardımcı olabilirim?'
        ],
        'services': [
            'Pro-Lance olarak web tasarım, mobil uygulama geliştirme, SEO optimizasyonu, dijital pazarlama ve e-ticaret çözümleri sunuyoruz.',
            'Hizmetlerimiz arasında modern web siteleri, mobil uygulamalar, SEO danışmanlığı ve dijital pazarlama stratejileri bulunmaktadır.',
            'Web tasarım, uygulama geliştirme, SEO ve dijital pazarlama alanlarında profesyonel hizmet veriyoruz.'
        ],
        'pricing': [
            'Fiyatlarımız proje kapsamına göre değişiklik göstermektedir. Detaylı fiyat teklifi için lütfen bizimle iletişime geçin.',
            'Her proje kendine özgü olduğu için özel fiyat teklifi hazırlıyoruz. İletişim formunu doldurarak teklif alabilirsiniz.',
            'Proje detaylarınızı paylaştığınızda size özel bir fiyat teklifi sunabiliriz.'
        ],
        'contact': [
            'Bizimle iletişime geçmek için web sitemizdeki iletişim formunu kullanabilir veya hasancakir@smartiasolutions.com adresine e-posta gönderebilirsiniz.',
            'İletişim sayfamızdan bize ulaşabilir veya doğrudan e-posta gönderebilirsiniz.',
            'Sorularınız için iletişim formunu doldurun, en kısa sürede size geri dönüş yapalım.'
        ],
        'default': [
            'Bu konuda size nasıl yardımcı olabilirim? Daha spesifik bir soru sorabilir misiniz?',
    ...
    'en': {
        'greeting': [
            ...
            'Hi there! I\'m here to answer your questions about Pro-Lance services.',
            'Welcome! How can I assist you today?'
        ],
        'services': [
            'Pro-Lance offers web design, mobile app development, SEO optimization, digital marketing, and e-commerce solutions.',
            'Our services include modern websites, mobile applications, SEO consulting, and digital marketing strategies.',
            'We provide professional services in web design, app development, SEO, and digital marketing.'
        ],
        'pricing': [
            'Our pricing varies based on project scope. Please contact us for a detailed quote.',
            'Since each project is unique, we prepare custom quotes. You can get a quote by filling out our contact form.',
            'We can provide you with a custom quote once you share your project details.'
        ],
        'contact': [
            'You can contact us using the contact form on our website or send an email to hasancakir@smartiasolutions.com.',
            'You can reach us through our contact page or send us a direct email.',
            'Fill out our contact form for your questions, and we\'ll get back to you shortly.'
        ],
        'default': [
            'How can I help you with this? Could you ask a more specific question?',
            'Please explain your question in more detail so I can assist you better.',
            'Could you provide more information about this topic?'
        ]
    }
}

def get_chatbot_response(message, language='tr'):
    ...
    if language == 'auto':
        ...
        turkish_words = ['merhaba', 'selam', 'nasıl', 'nedir', 'hakkında', 'fiyat', 'iletişim']
        english_words = ['hello', 'hi', 'how', 'what', 'about', 'price', 'contact']
        
        turkish_count = sum(1 for word in turkish_words if word in message_lower)
        english_count = sum(1 for word in english_words if word in message_lower)
        
        language = 'tr' if turkish_count >= english_count else 'en'
    
    responses = CHATBOT_RESPONSES.get(language, CHATBOT_RESPONSES['tr'])
    
    # Greeting detection
    greeting_keywords = ['merhaba', 'selam', 'hello', 'hi', 'hey', 'good morning', 'good afternoon']
    if any(keyword in message_lower for keyword in greeting_keywords):
        return random.choice(responses['greeting'])
    
    # Services detection
    service_keywords = ['hizmet', 'service', 'web', 'tasarım', 'design', 'uygulama', 'app', 'seo', 'pazarlama', 'marketing']
    if any(keyword in message_lower for keyword in service_keywords):
        return random.choice(responses['services'])
    
    # Pricing detection
    price_keywords = ['fiyat', 'price', 'ücret', 'cost', 'para', 'money', 'teklif', 'quote']
    if any(keyword in message_lower for keyword in price_keywords):
        return random.choice(responses['pricing'])
    
    # Contact detection
    contact_keywords = ['iletişim', 'contact', 'ulaş', 'reach', 'telefon', 'phone', 'email', 'mail']
...
def chat_message():
    ...
    try:
        ...
        try:
            ...
            save_chat_conversation(conversation_data)
        except Exception as e:
            logger.warning(f"Failed to save chat conversation: {str(e)}")
...
def create_chat_session():
    ...
    try:
        ...
        language = request.json.get('language', 'tr') if request.json else 'tr'
        ...
        greeting = random.choice(CHATBOT_RESPONSES[language]['greeting'])
...
def save_chat_conversation(data):
    """Save chat conversation to local storage"""
    try:
        os.makedirs('chat_logs', exist_ok=True)
        filename = f"chat_logs/conversation_{data['session_id']}_{datetime.now().strftime('%Y%m%d')}.json"
...
def save_chat_session(data):
    """Save chat session to local storage"""
    try:
        os.makedirs('chat_logs', exist_ok=True)
        filename = f"chat_logs/sessions.json"
